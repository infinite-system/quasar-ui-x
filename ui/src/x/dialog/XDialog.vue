...
function loadChange (newA, prevA) {
  ...
  log(() => ({ localShow: localShow.value, localComponent: localComponent.value, validComponent: validComponent.value, compare: newA?.toString() + '===' + prevA?.toString() }))
  if (localShow.value && validComponent.value && componentChanged(newA, prevA)) {
...
log(() => ({'fullConfig.load.fn': fullConfig.load.fn, 'loadFn': loadFn}))
...
async function watchLoad () {
  ...
  if (loadComponent.value && localShow.value) {
    ...
    try {
      ...
      log(() => ({ watchLoad, loadComponent: loadComponent.value, loadConfig: fullConfig.load }))
      ...
      log(() => ({'watchload resolvedImport': resolvedImport.value, 'loadFile': loadFile.value, loadFn }))
